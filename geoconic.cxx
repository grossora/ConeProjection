...
//-----------------------------------------------------------------------------------------------------------
//-------------------------CONTAINMENT INFO AND FUNCTIONS----------------------------------------------------
//-----------------------------------------------------------------------------------------------------------
...
bool geoconic::TPCContained(std::vector<double> pos , double fid ){
        auto geo = larutil::Geometry::GetME();
	auto x = geo->DetHalfWidth();
	auto y = geo->DetHalfHeight();
	auto z = geo->DetLength();
	...
	double zlo = 0+fid;
	double zhi = z-fid;
        if(pos[0]>xhi || pos[0]<xlo || pos[1]>yhi || pos[1]<ylo || pos[2]<zlo || pos[2]>zhi){
		return  false;}
	return true;
	}//geoconic::TPCContained


std::vector<double> geoconic::ForceTPCContained(std::vector<double> pos  , double fid){
	 auto geo = larutil::Geometry::GetME();
	auto x = geo->DetHalfWidth();
	auto y = geo->DetHalfHeight();
	auto z = geo->DetLength();
	...
	if(pos[0]>xhi) pos[0] = xhi;
	if(pos[0]<xlo) pos[0] = xlo;
	if(pos[1]>yhi) pos[1] = yhi;
	if(pos[1]<ylo) pos[1] = ylo;
	if(pos[2]>zhi) pos[2] = zhi;
	if(pos[2]<zlo) pos[2] = zlo;
...
//-----------------------------------------------------------------------------------------------------------
//-----------------------------------------------------------------------------------------------------------
	// This returns Polygon edges for the cone //EVEN IF it falls out of the volume 
	// If the cone is out of the volume it will pin the polygon points to the edges
		...
		// 5 Plane
		// 6 smoothness 
        std::vector<larutil::PxPoint> geoconic::FitConicalFeatures(const TVector3& Pos, const TVector3& dir, double Length, double OpeningAngle, int plane, int smoothness){
  ...
  for(double theta =0 ; theta<2*PI; theta+=PI/smoothness){
                ...
                double paramz = Pos.Z() + Length*Axis.Z()/Axis.Mag() + Radius*cos(theta)*avec.Z() + Radius*sin(theta)*bvec.Z();
		//TVector3 FillPoint;
		// NEED TO CHECK IF POINT IS IN TPC
		std::vector<double> testPoint;
                testPoint.push_back(paramx);
                testPoint.push_back(paramy);
                testPoint.push_back(paramz);
		bool PtContained = TPCContained(testPoint, 14);


		if(!PtContained){
		// IF IT IS NOT THEN FORCE IT TO THE EDGE
		std::vector<double> fixpt = ForceTPCContained(testPoint,14);
                auto pt =geom->Get2DPointProjectionCM(fixpt,plane);
                pConeFullHits.push_back(pt);
			} 
		if(PtContained){
                auto pt =geom->Get2DPointProjectionCM(testPoint,plane);
...
//-----------------------------------------------------------------------------------------------------------
  ...
  for(double theta =0 ; theta<2*PI; theta+=PI/smoothness){
                ...
                double paramz = Pos.Z() + Length*Axis.Z()/Axis.Mag() + Radius*cos(theta)*avec.Z() + Radius*sin(theta)*bvec.Z();
		//TVector3 FillPoint;
		std::vector<double> FillPoint;
                FillPoint.push_back(paramx);
                FillPoint.push_back(paramy);
                FillPoint.push_back(paramz);
